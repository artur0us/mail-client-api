import time, mailbox, base64, os, uuid, hashlib, html2text
...
def prepare_msg(msg_obj):
    ...
    msg_sent_at = int(time.mktime((dtparser.parse(msg_obj["Date"])).timetuple()))
        ...
        for idx, item in enumerate(msg_obj.get_payload()):
            if ("html" in item.get_content_type()) or ("html" in item.get_content_type()):
                ...
                if (item.get_filename() == None) or (item.get_filename() == "") or (item.get_filename() == " "):
                    ...
                    msg_attachments_as_base64.append(str(base64.b64encode(item.get_payload(decode = True)).decode('utf-8')))
