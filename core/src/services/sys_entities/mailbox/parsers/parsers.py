...
from src.services.sys_entities.mailbox.checkers.correctness.checkers import MBoxCorrectCheckersEntity
from src.services.sys_entities.mailbox.checkers.drafts.checkers import MboxDraftsCheckers
from src.services.sys_entities.mailbox.checkers.spam.checkers import MBoxSpamCheckers
from src.services.sys_entities.mailbox.cleaners.main import MBoxCleaners
from src.services.sys_entities.mailbox.mailbox import MBoxEntity

from src.services.sys_entities.mailbox.parsers.answers import Answers
...
class MBoxParsers:
  ...
  def get_all_msgs(mbox_file_path, delete_in_final):
    ...
    if MBoxEntity.is_mbox_locked:
      return Answers.basic_info["SVC_IS_BUSY"], {}
    ...
    if (
      ...
      # Delete messages
      pass
      threading.Thread(target=MBoxCleaners.delete_messages_by_keys, args=(mbox_file_path, msgs_keys_to_delete, )).start()
 
    if is_err_result:
      return Answers.basic_info["ALL_MSGS_PARSE_ERR"], {}
    return Answers.basic_info["OK"], all_msgs
  ...
  def parse_one_msg(raw_msg):
    ...
    if is_err_result:
      return Answers.basic_info["ONE_MSG_PARSE_ERR"], {}
    return Answers.basic_info["OK"], prepared_msg


  @staticmethod
  def get_one_msg_attachments(sent_at, message_id):
    all_msgs = MBoxParsers.get_all_msgs(MBoxSettings.USING_MBOX_FILE_PATH)
    msg_attachments_as_base64 = []
    ...
    return Answers.basic_info["OK"], msg_attachments_as_base64
